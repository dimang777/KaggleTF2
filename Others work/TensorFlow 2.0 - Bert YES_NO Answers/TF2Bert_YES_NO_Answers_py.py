...
on_kaggle_server = os.path.exists('../../')
...
with open('../bert-joint-baseline/bert_config.json','r') as f:
    config = json.load(f)
...
#         if tf.compat.dimension_value(input_shape[-1]) is None: # original
        if input_shape[-1] is None: # tf2
          raise ValueError("The last dimension of the inputs to "
                           "`TDense` should be defined. "
                           "Found `None`.")
#         last_dim = tf.compat.dimension_value(input_shape[-1]) # original
...
if not os.path.exists(eval_records):
    ...
    tokenizer = tokenization.FullTokenizer(vocab_file='../bert-joint-baseline/vocab-nq.txt', 
                                           do_lower_case=True)
...
candidates_dict = read_candidates('../../simplified-nq-test.jsonl')
